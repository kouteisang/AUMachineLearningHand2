...
def softmax(X):
    ...
    ### YOUR CODE HERE
    za = X.max(axis=1, keepdims=True)
    res = np.exp(X - za - np.log(np.exp(X - za).sum(axis=1, keepdims=True)))
    ### END CODE
    return res
...
class NetClassifier():
    ...
    def predict(self, X, params=None):
        ...
        relu_first = relu(first)
        second = np.dot(relu_first, W2) + b2
        pred = np.argmax(second, axis=1)
        ### END CODE
        return pred
...
if __name__ == '__main__':
    ...
    softmax_in = np.dot(xw1_out, W2) + b2
    nn = softmax(softmax_in)
    d_b2 = np.mean(nn-labels,axis=0).reshape(b2.shape)
    d_nn = nn-labels
    d_w2 = np.dot(xw1_out.T,d_nn)
    n = X.shape[0]
    # print('d_w2')
    # print(d_w2)
